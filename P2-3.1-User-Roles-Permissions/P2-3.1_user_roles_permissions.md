...
## Key URLs — Admin & User Management

All URLs are relative to the production base: `https://invenio.turath-project.com`

| Action | URL | Who |
|--------|-----|-----|
| Login | `/login/` | All users |
| Logout | `/logout/` | All users |
| Register new account | `/signup/` | Anyone (requires email confirmation) |
| Reset password | `/lost-password/` | All users |
| Change password | `/account/settings/security/` | Logged-in users |
| User profile settings | `/account/settings/profile/` | Logged-in users |
| **Administration panel** | `/administration/` | System Administrator only |
| Manage users (list, activate, deactivate, block) | `/administration/users/` | System Administrator only |
| Manage roles | `/administration/roles/` | System Administrator only |
| Upload new record (deposit form) | `/uploads/new` | Curator + Admin |
| My uploads (drafts & published) | `/me/uploads/` | Curator + Admin |
| Browse all records | `/search` | Public |

### Admin Panel — User Management Actions

From `/administration/users/`, an administrator can:
- **View** all registered users and their status
- **Activate** a user account (allow login)
- **Deactivate** a user account (block login, non-destructive)
- **Block** a user (permanent restriction)
- **Assign / remove roles** (e.g. promote to curator or admin)

`[ATTACH: screenshot — admin panel user management page]`
`[ATTACH: video clip — full walkthrough: login → /administration/users → create, activate, deactivate, assign role]`
